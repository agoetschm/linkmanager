...
class ForgotPassword @Inject()(
  ...
  def submit = silhouette.UnsecuredAction.async { implicit request =>
    ForgotPasswordForm.form.bindFromRequest.fold(
      ...
      email => {
        val result = Redirect(routes.LogIn.view()).flashing("info" -> Messages("reset.email.sent"))
        userDAO.findByEmail(email).flatMap {
          case Some(user) =>
            authTokenService.create(user.id).map { authToken =>
              val url = routes.ResetPassword.view(authToken.id).absoluteURL()
